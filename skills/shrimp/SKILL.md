...
복잡한 작업을 plan -> analyze -> split -> execute -> verify 흐름으로 처리한다. 세션 간 메모리를 유지해 과거 작업을 참조하고 중복을 방지한다.
...
- **작업이 끝난 후 다음 단계 진행 여부를 묻지 말 것 — 결과만 보여주고 멈춘다.**
- **사용자가 태스크 번호를 언급하면 (예: "3번", "#2") 해당 태스크를 즉시 실행한다. 확인 질문 없이 바로 시작한다.**
